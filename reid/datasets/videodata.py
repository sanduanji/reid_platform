...
class Mars(Dataset):
    ...
    """
    root = '/media/saber/DATASET/reid-demo/open-reid/examples/data/mars'
    ...
    def __init__(self, root, split_id=0, num_val=100, min_seq_len=0):
        ...
        fpath = osp.join(raw_dir, 'MARS-v160809.zip')
        if osp.isfile(fpath):
            raw_dir = osp.join(self.root, 'raw')
        ...
        def _process_data(names, meta_data, home_dir=None, relabel=False, min_seq_len=0):
            ...
            for tracklet_idx in range(num_tracklets):
                ...
                img_paths = [osp.join(self.root, 'raw', home_dir, img_name[:4], img_name) for img_name in img_names]
                ...
                for img_path in img_paths:
                    if osp.isfile(osp.join(images_dir, fname)):
                        continue
            ...
            return tracklets, num_tracklets, num_pids, num_imgs_per_tracklet, pids
        ...
        train, num_train_tracklets, num_train_pids, num_train_imgs, trainval_pids = \
            _process_data(train_names, track_train, home_dir='bbox_train', relabel=True, min_seq_len=min_seq_len)

        query, num_query_tracklets, num_query_pids, num_query_imgs, gallery_pids = \
            _process_data(test_names, track_query, home_dir='bbox_test', relabel=False, min_seq_len=min_seq_len)

        gallery, num_gallery_tracklets, num_gallery_pids, num_gallery_imgs, query_pids = \
            _process_data(test_names, track_gallery, home_dir='bbox_test', relabel=False, min_seq_len=min_seq_len)
    ...
    def _download_data(self):
        ...
        print("Downloading MARS dataset")
